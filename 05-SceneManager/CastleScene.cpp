...
CastleScene::CastleScene(int id, LPCWSTR filePath) :
	CScene(id, filePath)
{
	player = NULL;
...
void CastleScene::_ParseSection_SPRITES(string line)
{
...
void CastleScene::_ParseSection_SPRITES_PLUS(string line)
{
	vector<string> tokens = split(line);
...
void CastleScene::_ParseSection_ASSETS(string line)
{
	vector<string> tokens = split(line);
...
void CastleScene::_ParseSection_ANIMATIONS(string line)
{
	vector<string> tokens = split(line);
...
*/
void CastleScene::_ParseSection_OBJECTS(string line)
{
	vector<string> tokens = split(line);
...
void CastleScene::Update(DWORD dt)
{
...
void CastleScene::DropItem(int itemType, float x, float y)
{
	switch (itemType)
	{
	case 4:
...
bool CastleScene::IsGameObjectDeleted(const LPGAMEOBJECT& o) { return o == NULL; }

void CastleScene::PurgeDeletedObjects()
{
	vector<LPGAMEOBJECT>::iterator it;
	for (it = objects.begin(); it != objects.end(); it++)
	{
	...
	objects.erase(
		std::remove_if(objects.begin(), objects.end(), CastleScene::IsGameObjectDeleted),
		objects.end());
